...
const Overview: React.FC<TelemetryProps> = ({ cars: initialCars, drivers, telemetryData, onCarSelect, thresholds }) => {
    ...
    useEffect(() => {
        ...
        setAggregatedData(prev => {
            ...
            telemetryData.forEach(car => {
                ...
                if (car.speed > thresholds.speed) newViolations.speed++;
                if (car.rpm > thresholds.rpm) newViolations.rpm++;
                if (car.fuelFlow > thresholds.fuelFlow) newViolations.fuelFlow++;
                if (car.fuelPressure > thresholds.fuelPressure) newViolations.fuelPressure++;
                if (car.throttle > thresholds.throttle) newViolations.throttle++;
                if (car.ignitionTiming > thresholds.ignitionTiming) newViolations.ignitionTiming++;
                if (car.airflow > thresholds.airflow) newViolations.airflow++;
                if (car.lambda > thresholds.lambda) newViolations.lambda++;
                ...
                {displayCars.map(car => (
                    ...
                    >
                        ...
                        <div className="w-16 lg:w-20 2xl:w-24 flex flex-col items-center justify-center border-r border-white/5 bg-black/20 flex-shrink-0">
                            ...
                            <span className="text-[9px] lg:text-[10px] 2xl:text-xs font-bold text-zinc-500 truncate max-w-[50px] lg:max-w-[70px] text-center mt-1">
